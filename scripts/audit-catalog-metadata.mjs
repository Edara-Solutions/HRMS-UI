...
export function collectAuditCatalogMetadata() {
  ...
  // An event type holds one arm per payload version. The catalog knows one thing about the
  // event either way, and the newest version is the one that describes what it records now.
  const newestArmByEventType = new Map(
    armsOf(document, "PlatformAuditTrailPage").map((arm) => [
      arm.properties.eventType.enum[0],
      arm,
    ]),
  );
